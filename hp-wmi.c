...
static struct device *devm_platform_profile_register(struct device *dev, const char *name,
                                                     void *drvdata, const struct hp_wmi_ops *ops)
{
	int err;
...
static int hp_wmi_setup_fan_settings(struct hp_wmi_hwmon_priv *priv)
{
	...
	if (ret) {
		pr_info("HP WMI: Fan table query failed (%d), using max_rpm=%u\n",
			ret, priv->max_rpm);
	...
	if (fan_table->header.num_fans == 0) {
		pr_info("HP WMI: Fan table reports 0 fans, using max_rpm=%u\n",
			priv->max_rpm);
		return 0;
	}
	...
	for (i = 0; i < num_entries; i++) {
		...
		/*
		 * On some devices, the fan table is truncated with an all-zero
		 * row, hence we stop parsing here.
		 */
	...
	if (min_rpm == U8_MAX || max_rpm == 0) {
		pr_info("HP WMI: Fan table empty after parsing, using max_rpm=%u\n",
			priv->max_rpm);
		return 0;
	}
